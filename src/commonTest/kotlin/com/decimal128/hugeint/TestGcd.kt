...
class TestGcd {
    ...
    fun test1(bitLen: Int) {
        val x = HugeInt.fromRandom(bitLen, withRandomSign = true)
        val y = HugeInt.fromRandom(bitLen, withRandomSign = true)
        ...
        val k = HugeInt.fromRandom(Random.nextInt(29))
        val a = x * k
        val b = y * k
    ...
    fun testSymmetry(x: HugeInt, y: HugeInt) {
        val gcdXY = x.gcd(y)
        val gcdYX = y.gcd(x)
        assertEquals(gcdXY, gcdYX)
    }

    fun testIdempotence(x: HugeInt) {
        val gcdXX = x.gcd(x)
        assertEquals(gcdXX, x.abs())
    }

    fun testZero(x: HugeInt) {
        val gcdXZero = x.gcd(HugeInt.ZERO)
        val gcdZeroX = HugeInt.ZERO.gcd(x)
        assertEquals(gcdXZero, x.abs())
        assertEquals(gcdZeroX, x.abs())
    }

    fun testSigns(x: HugeInt, y: HugeInt) {
        val gcd0 = x.gcd(y)
    ...
    fun testProblemChild() {
        val x = HugeInt.from("75739105468096430")
        val y = HugeInt.from("112746730774794142")
        testSymmetry(x, y)
    }

    @Test
    fun testProblemChild2() {
        val x = HugeInt.from("4")
        val y = HugeInt.from("1")
        testSymmetry(x, y)
    ...
    fun testProblemChild3() {
        val x = HugeInt.from("-39")
        val y = HugeInt.from("-39")
        val k = HugeInt.from("115399892")
        val a = k * x
        val b = k * y
        val gcdAB = a.gcd(b)
        val gcdXY = x.gcd(y)
