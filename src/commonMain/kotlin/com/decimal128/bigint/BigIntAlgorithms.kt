 ...
 */
internal fun factorialImpl(n: Int): BigInt {
    if (n < 0)
 ...
 */
internal fun gcdImpl(a: BigInt, b: BigInt): BigInt {
    if (a.isZero())
        return b.abs()
    if (b.isZero())
        return a.abs()
 ...
 */
internal fun lcmImpl(a: BigInt, b: BigInt): BigInt {
    if (a.isZero() || b.isZero())
        return ZERO
    val aAbs = a.abs()
    val bAbs = b.abs()
    val gcd = gcdImpl(aAbs, bAbs)
    if (aAbs < bAbs)
 ...
 */
internal fun powImpl(base: BigInt, exp: Int): BigInt {
...
internal fun tryPowFastPath(base: BigIntNumber, exp: Int, mbi: MutableBigInt): Boolean {
    val resultSign = base.isNegative() && ((exp and 1) != 0)
    when {
        exp < 0 -> throw IllegalArgumentException("cannot raise BigInt to negative power:$exp")
        exp == 0 -> mbi.setOne()
 ...
 */
internal fun powLeftToRight(base: BigIntNumber, exp: Int, ret: MutableBigInt) {
    if (exp <= 2)
 ...
 */
internal fun isqrtImpl(radicand: BigIntNumber): BigInt {
 ...
 */
internal fun isPerfectSquare(radicand: BigIntNumber): Boolean {
    val isqrt = isqrtImpl(radicand)
 ...
 */
internal fun montgomeryRedc(t: Magia, tLen: Int, n: Magia, k: Int, np: UInt): Int {
